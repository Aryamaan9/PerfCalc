...
export function parseTrades(buf: Buffer): Trade[] {
...
export function parsePrices(buf: Buffer): PriceRecord[] {
  const wb = XLSX.read(buf, { type: "buffer", cellDates: false });
  const ws = wb.Sheets[wb.SheetNames[0]];
  const rows: Record<string, any>[] = XLSX.utils.sheet_to_json(ws, { defval: "", raw: false });
  return rows.map(r => {
...
export function parseCorporateActions(buf: Buffer): CorporateAction[] {
  const wb = XLSX.read(buf, { type: "buffer", cellDates: false });
  const ws = wb.Sheets[wb.SheetNames[0]];
  const rows: Record<string, any>[] = XLSX.utils.sheet_to_json(ws, { defval: "", raw: false });
  ...
  return rows.map(r => {
    ...
    return { date: d, symbol: sym, action, value } as CorporateAction;
